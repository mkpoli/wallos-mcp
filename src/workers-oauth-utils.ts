...
function approvalCopy(client: ClientInfo | null, server: ApprovalDialogOptions["server"]) {
	const redirectUris = (client?.redirectUris ?? [])
		...
		.filter((uri) => uri !== "");
	return {
		serverName: sanitizeText(server.name),
		clientName: client?.clientName ? sanitizeText(client.clientName) : "Unknown MCP Client",
		serverDescription: server.description ? sanitizeText(server.description) : "",
		consentNote: server.consentNote ? sanitizeText(server.consentNote) : "",
		logoUrl: server.logo ? sanitizeText(sanitizeUrl(server.logo)) : "",
		clientUri: client?.clientUri ? sanitizeText(sanitizeUrl(client.clientUri)) : "",
		policyUri: client?.policyUri ? sanitizeText(sanitizeUrl(client.policyUri)) : "",
		tosUri: client?.tosUri ? sanitizeText(sanitizeUrl(client.tosUri)) : "",
		contacts:
