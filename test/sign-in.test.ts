...
describe("who the sign-in admits", () => {
	...
	test("admits a host on the allowlist and stores the grant", async () => {
		...
		expect(await kv.get("account:https%3A%2F%2Fwallos.example.com@7")).not.toBeNull();
	});
	...
	test("refuses a new account once the cap is reached", async () => {
		const kv = memoryKv();
		await kv.put("account:https%3A%2F%2Fother.example.com@1", "already");
		const completed: PropsCapture[] = [];
	...
	test("lets a known account back in at the cap", async () => {
		const kv = memoryKv();
		await kv.put("account:https%3A%2F%2Fwallos.example.com@2", "already");
		const completed: PropsCapture[] = [];
